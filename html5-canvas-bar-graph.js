  ...
  // Draw method updates the canvas with the current display
	var draw = function (arr) {
	  ...
	  if (ctx.canvas.width !== that.width || ctx.canvas.height !== that.height) {
		...
		if(numOfBars > mod )
	  {
			ctx.canvas.height = that.height * (numOfBars/mod) + 50 ;
	  }	  
	  ...
	  for (i = 0; i < arr.length; i += 1) {
		...
		if (i < mod)
		{
			newHeight = that.height;
			z = i;
		}
		//change
		if (i > 1 && i%mod == 1)
		{
			graphAreaHeight = graphAreaHeight * 2.5;
		...
		ctx.fillRect(that.margin + z * that.width / numOfBars,
		...
		if (barHeight > border * 2) {
			...
			gradient.addColorStop(1-ratio, that.colors[z % that.colors.length]);
			gradient.addColorStop(1, "#ffffff");

			ctx.fillStyle = gradient;
			// Fill rectangle with gradient
			ctx.fillRect(that.margin + z * that.width / numOfBars + border,
		...
		try {
		  ctx.fillText(parseInt(arr[i],10),
			z * that.width / numOfBars + (that.width / numOfBars) / 2,
		 ...
		 // ctx.rotate(-0.1);
		  ...
		  try{
			ctx.fillText(that.xAxisLabelArr[i],
			  ...
			  /*ctx.fillText(that.xAxisLabelArr[i],
			  i*that.width / numOfBars - (that.width / numOfBars) * 1.2,
			  that.height - 100);*/
			} catch (ex) {}
